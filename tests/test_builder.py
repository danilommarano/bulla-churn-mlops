...
def test_balance_per_product_and_null_fill():
    X, y = _raw(
        2, ["Sao Paulo", "Sao Paulo"], [30, 40], [np.nan, 2000.0], [2, 4], [1, 0]
    )
    out = ChurnFeatureBuilder().fit(X, y).transform(X)
...
def test_geography_rate_is_learned_on_train_only():
    ...
    Xtr, ytr = _raw(
        2, ["Sao Paulo", "Sao Paulo"], [30, 40], [100.0, 200.0], [1, 1], [1, 0]
    )
    Xte, _ = _raw(
        2, ["Sao Paulo", "Sao Paulo"], [30, 40], [100.0, 200.0], [1, 1], [1, 1]
    )
